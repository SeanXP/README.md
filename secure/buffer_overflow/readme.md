...
1. 先执行exploit.c编译的程序，其功能是生成一个badfile，这个badfile大小为517字节，正好存放exploit.c程序中的buffer字符数组。
2. buffer数组的内容，程序开始先用memset函数全部置为0x90（汇编语言NOP指令的十六进制数据就是0x90），然后在buffer的开头写入一段数据\x90\x90...\x??\x??，前面的\x90一共24个，正好12个字节，对应stack.c文件中要拷贝的buffer[12]；
3. 将shellcode代码（这里是获取/bin/sh）通过strcpy写在buffer+100的位置；因此只要得到buffer的地址，就知道了shellcode的地址。
4. 执行完exploit程序，然后执行stack程序。stack程序读取badfile文件，读取517字节，放在str数组中，然后在bof函数中将str数组拷贝给buffer。因为没有检查越界，strcpy会将str数组所有的517字节都拷贝到buffer中，覆盖了前面的12个字节，之后的返回地址正好也被覆盖。因此在bof函数返回时，就会跳转到预设的地址，这里是shellcode的地址；
5. 之前通过指令sudo sysctl -w kernel.randomize_va_space=0取消了系统的地址空间随机化，从而每次执行程序的地址都是确定的。这里通过gdb调试程序，查看汇编代码，找到stack.c中str[]数组的分配地址，其str+100的位置正存放着shellcode代码。
